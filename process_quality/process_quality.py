...
def _update_accum_sheet(wb, data: dict, year: int, month: int):
    ...
    # 월 열 채우기
    _put("생산수", month_col, prod_total)
    _put("불량수",  month_col, defect_total)
    _put("불량율",  month_col, rate)
    _put("시그마",  month_col, round(sigma, 2))
    ...
    if accum_col:
        ...
        _put("생산수", accum_col, new_prod)
        _put("불량수",  accum_col, new_defect)
        _put("불량율",  accum_col, new_rate)
